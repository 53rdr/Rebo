...
async def start(update, context):
    ...
    await update.message.reply_text("👋 Welcome to *Rebatrix*!\n\nEarn *cashback on every forex trade* — win or lose.\n\nWhat would you like to do?",parse_mode="Markdown",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💰 What are rebates?",callback_data="explain")],[InlineKeyboardButton("✅ I know — let's get started",callback_data="onboard")],[InlineKeyboardButton("🧮 Calculate my rebates",callback_data="calc")]]))
...
async def button(update, context):
    ...
    u = USER[uid]
    home = InlineKeyboardMarkup([[InlineKeyboardButton("💰 What are rebates?",callback_data="explain")],[InlineKeyboardButton("✅ I know — let's get started",callback_data="onboard")],[InlineKeyboardButton("🧮 Calculate my rebates",callback_data="calc")]])

    if data == "home":
        USER[uid] = {}
        await q.edit_message_text("👋 Welcome to *Rebatrix*!\n\nEarn *cashback on every forex trade* — win or lose.\n\nWhat would you like to do?",parse_mode="Markdown",reply_markup=home)
    elif data == "explain":
        await q.edit_message_text("📚 *What are forex rebates?*\n\nEvery time you open a trade, your broker earns a spread or commission.\n\nRebatrix shares a portion back with *you* automatically.\n\n✅ You trade as normal\n✅ Cashback on every trade\n✅ Works on winning AND losing trades\n\nFree to join. No catch.",parse_mode="Markdown",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Got it — let's go",callback_data="onboard")],[InlineKeyboardButton("🧮 Calculate my rebates",callback_data="calc")],[InlineKeyboardButton("🏠 Main menu",callback_data="home")]]))
    elif data == "onboard":
        u["flow"] = "onboard"
        await q.edit_message_text("🔍 *Step 1 — Which broker do you trade with?*",parse_mode="Markdown",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Exness",callback_data="ob_exness"),InlineKeyboardButton("IC Markets",callback_data="ob_icmarkets")],[InlineKeyboardButton("XM",callback_data="ob_xm")],[InlineKeyboardButton("🏠 Main menu",callback_data="home")]]))
    elif data.startswith("ob_"):
        u["broker"] = data.replace("ob_","")
        await q.edit_message_text("💵 *Step 2 — What is your account size?*",parse_mode="Markdown",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Under $500",callback_data="os_1")],[InlineKeyboardButton("$500 – $2,500",callback_data="os_2")],[InlineKeyboardButton("$2,500 – $10,000",callback_data="os_3")],[InlineKeyboardButton("$10,000 – $50,000",callback_data="os_4")],[InlineKeyboardButton("$50,000 – $1M+",callback_data="os_5")],[InlineKeyboardButton(" Main menu",callback_data="home")]]))
